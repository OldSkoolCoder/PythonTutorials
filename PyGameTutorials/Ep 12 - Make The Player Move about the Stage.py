...
startScrollingRightPosX = (screenWidth - playerWidth - (playerVelocity * 1.5))
startScrollingLeftPosX = (playerVelocity * 1.5)
...
while running:
    ...
    elif keys[pygame.K_RIGHT] and playerX < (stageWidth - playerWidth - playerVelocity):
        playerX += playerVelocity
    ...
    spritePosX = playerX + stagePosX

    if spritePosX < startScrollingLeftPosX:
        spritePosX = startScrollingLeftPosX
        stagePosX += playerVelocity
    elif spritePosX > startScrollingRightPosX:
        spritePosX = startScrollingRightPosX
        stagePosX -= playerVelocity
    
    bgRelX = stagePosX % bgWidth

    # Draw Section
    window.blit(background, (bgRelX - bgWidth ,0))
    if bgRelX < screenWidth:
        window.blit(background, (bgRelX , 0))
    window.blit(characterImage, (spritePosX, playerY))
